...
Two gotchas hit along the way: the unbound-generic recursion initially looped forever (ConstructUnboundGenericType()  
 is idempotent on an unbound generic → test-host stack overflow), and the !IsGenericTypeDefinition guard doesn't exist  
 on INamedTypeSymbol in Roslyn 4.14 — fixed with !IsUnboundGenericType.
